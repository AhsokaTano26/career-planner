function enhance(select:HTMLSelectElement){
  ...
  const render=()=>{const current=select.selectedOptions[0];trigger.innerHTML=`<span>${current?.textContent||'请选择'}</span><i>⌄</i>`;menu.replaceChildren(...[...select.options].map(option=>{const button=document.createElement('button');button.type='button';button.textContent=option.text;button.className=option.selected?'selected':'';button.disabled=option.disabled;button.onclick=()=>{select.value=option.value;select.dispatchEvent(new Event('change',{bubbles:true}));close();render()};return button}))}
