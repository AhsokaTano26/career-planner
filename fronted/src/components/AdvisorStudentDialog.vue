...
const emit = defineEmits<{ close: []; submit: [payload: { content: string; adviceType: Guidance['adviceType']; suggestedTask?: string; retestReason?: string }] }>()
type RecordValue = Record<string, unknown>
const asRecord = (value: unknown): RecordValue => value && typeof value === 'object' && !Array.isArray(value) ? value as RecordValue : {}
const asList = (value: unknown): RecordValue[] => Array.isArray(value) ? value.map(asRecord) : []
const detail = computed(() => asRecord(props.student.detail))
const profile = computed(() => asRecord(detail.value.profile))
const portrait = computed(() => asRecord(detail.value.portrait))
const goal = computed(() => asRecord(detail.value.goal))
const plan = computed(() => asRecord(detail.value.plan))
const recommendation = computed(() => asRecord(detail.value.recommendation))
const tasks = computed(() => asList(detail.value.tasks))
const reviews = computed(() => asList(detail.value.reviews))
const recommendations = computed(() => asList(recommendation.value.results))
...
<template>
  <BaseModal @close="emit('close')">
    <section class="modal-card advisor-detail-modal">
      <header><div><p class="eyebrow">学生详情</p><h2>{{ String(profile.name || '学生详情') }}</h2><p>{{ String(profile.className || '未填写班级') }} · {{ student.id }}</p></div><button class="outline-btn" @click="emit('close')">关闭</button></header>
      <div class="advisor-detail-scroll">
        ...
        <section v-if="portrait.summary || portrait.completeness !== undefined" class="advisor-detail-section"><h3>能力画像</h3><p>{{ String(portrait.summary || '暂无画像摘要') }}</p><div class="detail-tags"><span v-for="item in (Array.isArray(portrait.strengths) ? portrait.strengths : [])" :key="String(item)">优势：{{ item }}</span><span v-for="item in (Array.isArray(portrait.explore) ? portrait.explore : [])" :key="String(item)">待探索：{{ item }}</span></div></section>
        <section class="advisor-detail-section detail-split"><div><h3>目标与计划</h3><p><b>主目标：</b>{{ String(asRecord(goal.primary).name || '尚未设定') }}</p><p><b>备选目标：</b>{{ String(asRecord(goal.backup).name || '尚未设定') }}</p><p><b>计划：</b>{{ String(plan.goalSummary || '尚未制定') }}</p><p><b>状态：</b>{{ statusLabel(String(plan.status || '')) }}</p></div><div><h3>推荐方向</h3><p v-for="item in recommendations.slice(0, 3)" :key="String(item.directionId)"><b>第 {{ item.rank ?? '—' }} 名</b> {{ String(item.directionId || '方向') }} · {{ item.score ?? '—' }} 分</p><p v-if="!recommendations.length">暂无推荐结果。</p></div></section>
