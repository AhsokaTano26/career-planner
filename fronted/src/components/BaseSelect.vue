...
<template><div ref="root" class="base-select" :class="{open,disabled}"><input class="base-select-value" tabindex="-1" :value="modelValue" :required="required" @invalid="open=true"><button type="button" class="base-select-trigger" :aria-expanded="open" aria-haspopup="listbox" :disabled="disabled" @click="toggle" @keydown="keydown"><span :class="{placeholder:!selected}">{{selected?.label||placeholder}}</span><i aria-hidden="true">⌄</i></button><Transition name="dropdown"><div v-if="open" class="base-select-menu" role="listbox"><button v-for="option in options" :key="option.value" type="button" role="option" :aria-selected="option.value===modelValue" :class="{selected:option.value===modelValue}" @click="choose(option.value)">{{option.label}}</button></div></Transition></div></template>
