...
async function submit(){const ok=action.value==='profile'?await saveAccount(name.value):action.value==='password'?await changePassword(oldPassword.value,newPassword.value):true;if(ok)close()}
</script>
