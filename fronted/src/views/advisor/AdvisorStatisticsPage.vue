...
const props=defineProps<{students:AdvisorStudent[];loading:boolean;error:string}>()
...
const averageCompleteness=computed(()=>average(props.students.map(student=>student.completeness)))
const averagePlanRate=computed(()=>average(props.students.map(student=>student.planRate)))
const assessedCount=computed(()=>props.students.filter(student=>student.assessed).length)
const priorityCount=computed(()=>props.students.filter(student=>student.status==='review'||student.status==='late').length)
const classDistribution=computed(()=>distribute(props.students,student=>student.className||'未填写班级'))
const pathDistribution=computed(()=>distribute(props.students,student=>pathName(student.path)))
const completenessDistribution=computed(()=>{
  ...
  return ranges.map(([label,match])=>({label,count:props.students.filter(student=>match(student.completeness??0)).length}))
})
const reviewDistribution=computed(()=>{
  ...
  return ranges.map(([label,match])=>({label,count:props.students.filter(match).length}))
})
...
<template>
  <PageHeader eyebrow="辅导工作" title="群体统计" description="按班级、发展路径、档案完整度与复盘时效，了解所带学生的整体情况。">
    <template #actions><button class="outline-btn" @click="emit('refresh')">刷新统计</button></template>
  </PageHeader>
