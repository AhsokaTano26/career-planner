...
const questionnaireVersions = ref<Row[]>([]), questionnaireTitle = ref(''), questionnaireDetail = ref<Row|null>(null), questionnaireBusy = ref(false)
...
async function openQuestionnaireVersions(row:Row){ questionnaireTitle.value=String(row.name||row.typeName||rowId(row)); questionnaireDetail.value=null; questionnaireBusy.value=true; try{questionnaireVersions.value=await api.admin.questionnaireVersions(rowId(row)) as Row[]}catch(e){emit('notice',getErrorMessage(e));questionnaireVersions.value=[]}finally{questionnaireBusy.value=false} }
