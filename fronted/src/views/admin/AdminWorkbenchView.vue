...
const editorSpecs:Record<string,Field[]> = {
  ...
  whitelist:[{key:'studentNo',label:'学号',required:true},{key:'className',label:'班级'},{key:'verifyCode',label:'初始校验码',placeholder:'留空则由服务端生成'}],
...
function summary(row:Row){ return Object.entries(row).filter(([key,v])=>!['id','userId','relationId','tagId','directionId','templateId','status','name','username','studentNo','verifyCode','password','passwordHash'].includes(key)&&v!==null&&v!==undefined&&v!=='').slice(0,4) }
function detailLines(row:Row){
  ...
  if(props.module==='relations') return [row.advisorName?`辅导员：${row.advisorName}`:`辅导员 ID：${value(row.advisorId)}`, row.studentName?`学生：${row.studentName}`:`学生 ID：${value(row.studentId)}`, row.createdAt?`建立于 ${dateTime(row.createdAt)}`:'']
...
async function save(){ submitting.value=true; try { const data=payload(), id=editing.value?rowId(editing.value):''
  ...
  else if(props.module==='whitelist') await api.admin.createWhitelist(data)
...
async function remove(row:Row){ if(!confirm('确定删除这条记录吗？')) return; try { const id=rowId(row); if(props.module==='whitelist') await api.admin.deleteWhitelist(id); else await api.admin.deleteRelation(id); emit('notice','已删除'); await load() } catch(e){emit('notice',getErrorMessage(e))} }
...
<template>
  ...
  <section v-else class="card data-list-card admin-workbench">
    ...
    <template v-else-if="props.module!=='weights'"><div class="list-summary"><span>共 <b>{{ total }}</b> 条记录</span><span v-if="isPage">第 {{ page }} / {{ totalPages }} 页</span></div><div class="enhanced-list"><div class="list-columns"><span v-for="heading in columnLabels()" :key="heading">{{ heading }}</span></div><article v-for="row in rows" :key="rowId(row)"><small v-if="props.module==='curricula'&&curriculumTab==='items'"><input v-model="selectedItems" type="checkbox" :value="rowId(row)"/></small><small v-else>{{ rowId(row) || '—' }}</small><div><b>{{ rowTitle(row) }}</b><span>{{ detailLines(row).filter(Boolean).join(' · ') || '—' }}</span></div><div class="row-actions"><em v-if="rowStatus(row)!=='—'" :class="rowStatusClass(row)">{{ rowStatus(row) }}</em><button v-if="props.module==='users'||props.module==='abilities'||props.module==='templates'||(props.module==='admin-directions')" class="outline-btn" @click="openEdit(row)">编辑</button><button v-if="props.module==='whitelist'||props.module==='relations'" class="outline-btn" @click="remove(row)">删除</button><button v-if="props.module==='admin-directions'" class="outline-btn" @click="changeDirectionStatus(row)">{{ String(row.status)==='PUBLISHED'?'停用':'发布' }}</button><button v-if="props.module==='exports'" class="outline-btn" :disabled="String(row.status)!=='COMPLETED'" @click="download(row)">下载</button><button v-if="props.module==='curricula'&&curriculumTab==='jobs'" class="outline-btn" @click="openJob(row,true)">查看并校核</button><button v-if="props.module==='curricula'&&curriculumTab==='items'" class="outline-btn" @click="openEdit(row,'curriculumItem')">审核编辑</button></div></article><p v-if="!rows.length" class="empty">暂无记录。</p></div><div v-if="isPage&&totalPages>1" class="admin-pagination"><button class="outline-btn" :disabled="page<=1" @click="nextPage(-1)">上一页</button><span>{{ page }} / {{ totalPages }}</span><button class="outline-btn" :disabled="page>=totalPages" @click="nextPage(1)">下一页</button></div></template>
  </section>
  <div v-if="modal" class="modal-mask"><form class="modal-card admin-editor" @submit.prevent="save"><p class="eyebrow">{{ editing ? '编辑' : '新建' }}</p><h2>{{ editKind==='curriculumItem'?'审核课程条目':editKind==='curriculumPublish'?'发布培养方案':title }}</h2><label v-for="field in currentFields" :key="field.key">{{ field.label }}<select v-if="field.type==='select'" v-model="form[field.key]" :required="field.required"><option value="" disabled>请选择</option><option v-for="option in field.options" :key="option" :value="option">{{optionText(option)}}</option></select><textarea v-else-if="field.type==='textarea'" v-model="form[field.key]" :required="field.required" :placeholder="field.placeholder"/><input v-else v-model.trim="form[field.key]" :type="field.type==='number'?'number':'text'" :min="field.type==='number'&&(['interest','values','ability','academic','tendency','practice','minConfidence'].includes(field.key)||field.key.startsWith('target'))?0:undefined" :max="field.type==='number'&&(['interest','values','ability','academic','tendency','practice','minConfidence'].includes(field.key)||field.key.startsWith('target'))?1:undefined" step="any" :required="field.required" :placeholder="field.placeholder"/></label><div><button type="button" class="outline-btn" @click="modal=false">取消</button><button class="primary-btn" :disabled="submitting">{{submitting?'正在保存…':'保存'}}</button></div></form></div>
</template>
