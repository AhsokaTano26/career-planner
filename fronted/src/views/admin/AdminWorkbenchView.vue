...
const editorSpecs:Record<string,Field[]> = {
  ...
  questionnaires:[{key:'type',label:'问卷类型',required:true},{key:'name',label:'问卷名称',required:true},{key:'typeName',label:'类型展示名'},{key:'icon',label:'图标'},{key:'minutes',label:'预计分钟',type:'number'},{key:'tip',label:'填写提示',type:'textarea'},{key:'changeNote',label:'修改说明',type:'textarea'}],
  questionnaireVersion:[{key:'questionnaireId',label:'问卷编号',required:true},{key:'type',label:'问卷类型',required:true},{key:'name',label:'问卷名称',required:true},{key:'typeName',label:'类型展示名'},{key:'minutes',label:'预计分钟',type:'number'},{key:'tip',label:'填写提示',type:'textarea'},{key:'changeNote',label:'版本说明',type:'textarea'}],
...
async function save(){ submitting.value=true; try { const data=payload(), id=editing.value?rowId(editing.value):''
  ...
  else if(editKind.value==='questionnaireVersion') { const {questionnaireId,...version}=data as Record<string,unknown>; await api.admin.createQuestionnaireVersion(String(questionnaireId),version) }
...
async function changeQuestionnaireStatus(row:Row){ const status=String(row.status)==='PUBLISHED'?'DISABLED':'PUBLISHED'; try{await api.admin.updateQuestionnaireStatus(rowId(row),{status});emit('notice',`问卷已${status==='PUBLISHED'?'发布':'停用'}`);await load()}catch(e){emit('notice',getErrorMessage(e))} }
async function publishPrompt(row:Row){ try{await api.admin.publishPrompt(rowId(row));emit('notice','提示词版本已发布');await load()}catch(e){emit('notice',getErrorMessage(e))} }
async function openQuestionnaireVersions(row:Row){ questionnaireId.value=rowId(row); questionnaireTitle.value=String(row.name||row.typeName||questionnaireId.value); questionnaireDetail.value=null; publishVersionId.value=''; questionnaireBusy.value=true; try{questionnaireVersions.value=await api.admin.questionnaireVersions(questionnaireId.value) as Row[]}catch(e){emit('notice',getErrorMessage(e));questionnaireVersions.value=[]}finally{questionnaireBusy.value=false} }
async function openQuestionnaireVersion(row:Row){ questionnaireBusy.value=true; try{questionnaireDetail.value=await api.admin.questionnaireVersion(String(row.questionnaireId||row.id||''),String(row.id||row.versionId||'')) as Row}catch(e){emit('notice',getErrorMessage(e))}finally{questionnaireBusy.value=false} }
async function publishQuestionnaireVersion(row:Row){ questionnaireBusy.value=true; try{await api.admin.publishQuestionnaireVersion(String(row.questionnaireId||''),String(row.id||row.versionId||''));emit('notice','问卷版本已发布');await openQuestionnaireVersions({id:row.questionnaireId,name:questionnaireTitle.value})}catch(e){emit('notice',getErrorMessage(e))}finally{questionnaireBusy.value=false} }
function createQuestionnaireVersion(){ editKind.value='questionnaireVersion'; editing.value=null; form.value={questionnaireId:questionnaireId.value}; modal.value=true }
async function publishSelectedQuestionnaireVersion(){ if(!questionnaireId.value||!publishVersionId.value)return; await publishQuestionnaireVersion({questionnaireId:questionnaireId.value,id:publishVersionId.value}) }
...
<template>
  ...
  <section v-else class="card data-list-card admin-workbench">
    ...
    <template v-else-if="props.module!=='weights'"><div class="list-summary"><span>共 <b>{{ total }}</b> 条记录</span><span v-if="isPage">第 {{ page }} / {{ totalPages }} 页</span></div><div class="enhanced-list"><div class="list-columns"><span v-for="heading in columnLabels()" :key="heading">{{ heading }}</span></div><article v-for="(row,index) in rows" :key="rowId(row)" :style="{ '--i': index }"><small v-if="props.module==='curricula'&&curriculumTab==='items'"><input v-model="selectedItems" type="checkbox" :value="rowId(row)"/></small><small v-else>{{ rowId(row) || '—' }}</small><div><b>{{ rowTitle(row) }}</b><span>{{ detailLines(row).filter(Boolean).join(' · ') || '—' }}</span></div><div class="row-actions"><em v-if="rowStatus(row)!=='—'" :class="rowStatusClass(row)">{{ rowStatus(row) }}</em><button v-if="props.module==='users'||props.module==='abilities'||props.module==='templates'||props.module==='admin-directions'||props.module==='questionnaires'" class="outline-btn" @click="openEdit(row)">编辑</button><button v-if="props.module==='whitelist'||props.module==='relations'" class="outline-btn" @click="askDelete(row)">删除</button><button v-if="props.module==='admin-directions'" class="outline-btn" @click="changeDirectionStatus(row)">{{ String(row.status)==='PUBLISHED'?'停用':'发布' }}</button><button v-if="props.module==='questionnaires'" class="outline-btn" @click="changeQuestionnaireStatus(row)">{{ String(row.status)==='PUBLISHED'?'停用':'发布' }}</button><button v-if="props.module==='questionnaires'" class="outline-btn" @click="openQuestionnaireVersions(row)">版本</button><button v-if="props.module==='prompts'" class="outline-btn" @click="publishPrompt(row)">发布</button><button v-if="props.module==='exports'" class="outline-btn" :disabled="String(row.status)!=='COMPLETED'" @click="download(row)">下载</button><button v-if="props.module==='curricula'&&curriculumTab==='jobs'" class="outline-btn" @click="openJob(row,true)">查看并校核</button><button v-if="props.module==='curricula'&&curriculumTab==='items'" class="outline-btn" @click="openEdit(row,'curriculumItem')">审核编辑</button></div></article><p v-if="!rows.length" class="empty">暂无记录。</p></div><div v-if="isPage&&totalPages>1" class="admin-pagination"><button class="outline-btn" :disabled="page<=1" @click="nextPage(-1)">上一页</button><span>{{ page }} / {{ totalPages }}</span><button class="outline-btn" :disabled="page>=totalPages" @click="nextPage(1)">下一页</button></div></template>
  </section>
  ...
  <Transition name="modal"><BaseModal v-if="questionnaireTitle" @close="questionnaireTitle='';questionnaireDetail=null"><section class="modal-card admin-editor"><p class="eyebrow">问卷版本</p><h2>{{questionnaireTitle}}</h2><div class="form-actions"><button class="outline-btn" type="button" @click="createQuestionnaireVersion">创建版本</button><input v-model.trim="publishVersionId" placeholder="版本编号"><button class="outline-btn" type="button" :disabled="!publishVersionId||questionnaireBusy" @click="publishSelectedQuestionnaireVersion">发布版本</button></div><p v-if="questionnaireBusy">正在读取…</p><div v-else class="simple-list"><button v-for="version in questionnaireVersions" :key="String(version.id||version.versionId)" type="button" @click="openQuestionnaireVersion(version)"><b>{{version.version||version.id}}</b><span>{{version.status||'—'}} · {{version.createdAt||''}}</span></button><p v-if="!questionnaireVersions.length" class="empty">暂无版本记录。</p></div><pre v-if="questionnaireDetail">{{JSON.stringify(questionnaireDetail,null,2)}}</pre><button class="outline-btn" type="button" @click="questionnaireTitle='';questionnaireDetail=null">关闭</button></section></BaseModal></Transition>
</template>
