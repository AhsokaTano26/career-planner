...
## Key Points (Linux 容器，本仓库当前环境）

- **CLI 调用**：直接 `npx apifox`（或全局 `apifox`），无 `.cmd/.ps1` 问题；输出落盘同样用 `> file` 保原始 UTF-8 字节，Python 侧 `decode("utf-8-sig")`。
- **Python**：系统 `python3`（3.10）跑仓库脚本（标准库 only）；career-ai 相关用 `~/devtools/python/bin/python`（3.12）。
- **环境变量**：`source ~/devtools/env.sh` 后再跑脚本。
