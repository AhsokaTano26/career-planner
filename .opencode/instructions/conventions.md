...
- **接口与鉴权**：统一前缀 `/api/v1`；JWT 认证（Spring Security + JJWT），登录接口 `POST /api/v1/auth/login` 获取 token，其余接口带 `Authorization: Bearer <token>`；涉及接口改动前先对照 `docs/openapi/career-core-apis-live.yaml` 与 `docs/openapi/career-core-apis-live.yaml`。
